    ...
    for(  iy = 2; iy <= my+1;  iy++ )
      { for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           ...
           if(ixp2 > nx-1) ixp2=ixp2-nx ;
           if(ixm2 < 0   ) ixm2=ixm2+nx ;
...
//        if(iym2 < 0   ) iym2=iym2+ny ;
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           ...
           adv[iy][ix]= adv[iy][ix]
	              +vv*(8.*(h[iyp1][ix]-h[iym1][ix])-(h[iyp2][ix]-h[iym2][ix]))/(12.*dy) ;
          }
      } 
...
//        if(iym2 < 0   ) iym2=iym2+ny ;
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           ...
           adv[iy][ix]= adv[iy][ix]
	              +vv*(8.*(u[iyp1][ix]-u[iym1][ix])-(u[iyp2][ix]-u[iym2][ix]))/(12.*dy) ;
          }
      } 
...
//        if(iym2 < 0   ) iym2=iym2+ny ;
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           vv=v[iy][ix]  ;
           adv[iy][ix]= adv[iy][ix]
	              +vv*(8.*(v[iyp1][ix]-v[iym1][ix])-(v[iyp2][ix]-v[iym2][ix]))/(12.*dy) ;
          }
      } 
//
  } 
    ...
    for(  iy = 2; iy <= my+1;  iy++ )
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           ...
           if(ixp2 > nx-1) ixp2=ixp2-nx ;
           if(ixm2 < 0   ) ixm2=ixm2+nx ;
           diff[iy][ix] = rh[iy][ixm2]-4.*rh[iy][ixm1]
		    +6.*rh[iy][ix]-4.*rh[iy][ixp1]+rh[iy][ixp2] ;
          }
      }
// y
    for(  iy = 2; iy <= my+1;  iy++ )
...
//        if(iym2 < 0   ) iym2=iym2+ny ;
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
          diff[iy][ix]= diff[iy][ix]+rh[iym2][ix]-4.*rh[iym1][ix]
		  +6.*rh[iy][ix]-4.*rh[iyp1][ix]+rh[iyp2][ix] ;
          }
      } 
//
  } 
...
//        if(iym1 < 0   ) iym1=iym1+ny ;
        ...
        for(  ix = 0; ix <= nx-1;  ix++ )
          { 
           ...
           tend_u[iy][ix]= tend_u[iy][ix]-f_u[iy]* 
            (v_b[iy][ix]+v_b[iyp1][ix]+v_b[iy][ixm1]+v_b[iyp1][ixm1])*0.25 ;

           tend_v[iy][ix]= tend_v[iy][ix]+f_v[iy]* 
            (u_b[iy][ix]+u_b[iym1][ix]+u_b[iy][ixp1]+u_b[iym1][ixp1])*0.25 ;
          }
      } 
//
  } 

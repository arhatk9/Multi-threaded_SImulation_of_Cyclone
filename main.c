...
/************************************************/
    ...
    MPI_Init( &argc, &argv); //Initialization of MPI execution environment.
    MPI_Comm_size(MPI_COMM_WORLD, &nproc); // Total no. of threads == nproc.
    MPI_Comm_rank(MPI_COMM_WORLD, &myid); // current theread == myid.


    int *dims = (int *) malloc( nproc* sizeof(int)); // Allocating memory to the arrays.
    int *disps = (int *) malloc( nproc* sizeof(int)); 

    init_myid(myid,&j_st,&j_en,&my,dims,disps,ny,nproc); // Calculation of init_myid.
...
// here, allocating the memroy chunk to arrays for calculation using MPI.
...
/************************************************/
    ...
    MPI_Barrier(MPI_COMM_WORLD); // MPI Barrier to block the caller until all processes in the communicator calls it.
    ...
    if (iys >= j_st && iys <=j_en)
     ...
     for(  iy = 0; iy <= my+3;  iy++ ) // Programming initialization of 2D arrays/vectors (matrices)
        { for(  ix = 0; ix <= nx-1;  ix++ )
             { h_b[iy][ix]= h_a[iy][ix] ;
               u_b[iy][ix]= u_a[iy][ix] ;
               v_b[iy][ix]= v_a[iy][ix] ;
...
//////
          ...
          diffusion(h_b,diff_h,nx,my) ; // Diffusion calculation for h_b
          diffusion(u_b,diff_u,nx,my) ; // Diffusion calculation for u_b
          diffusion(v_b,diff_v,nx,my) ; // Diffusion calculation for v_b 

          advection_h(u_b,v_b,h_b,adv_h,dx,dy,nx,my) ; //Advection along h calculation
          advection_u(u_b,v_b,adv_u,dx,dy,nx,my) ; // Advection along u calculation 
          advection_v(u_b,v_b,adv_v,dx,dy,nx,my) ; //Advection along v calculation
//
          for(  iy = 2; iy <= my+1;  iy++ )
            {for(  ix = 0; ix <= nx-1;  ix++ )
                  { 
                    tend_u[iy][ix] = g*dhdx[iy][ix] + adv_u[iy][ix] + gam*diff_u[iy][ix]; // Tendency term calculation for u
                    tend_v[iy][ix] = g*dhdy[iy][ix] + adv_v[iy][ix] + gam*diff_v[iy][ix] ; // Tendency term calculation for v
                    tend_h[iy][ix] = (h0+h_b[iy][ix])*(dudx[iy][ix] + dvdy[iy][ix]) // Tendency term calculation for h
                                      + adv_h[iy][ix] + gam*diff_h[iy][ix] ;
                  }
            }

          coriolis(tend_u,tend_v,u_b,v_b,f_u,f_v,nx,my)  ; // Coriolis parameter calculation

//
          for(  iy = 2; iy <= my+1;  iy++ )
             { for(  ix = 0; ix <= nx-1;  ix++ )
                  { 
                    h_b[iy][ix]=((1.-sig_h[iy][ix]*at*0.5)*h_a[iy][ix]-at*tend_h[iy][ix])
			        /(1.+sig_h[iy][ix]*at*0.5)  ;
                    u_b[iy][ix]=((1.-sig_u[iy][ix]*at*0.5)*u_a[iy][ix]-at*tend_u[iy][ix])
		                /(1.+sig_u[iy][ix]*at*0.5)  ;
                    v_b[iy][ix]=((1.-sig_v[iy][ix]*at*0.5)*v_a[iy][ix]-at*tend_v[iy][ix])
		                /(1.+sig_v[iy][ix]*at*0.5)  ;
...
/*********************** END rk3 loop *************************/
       ...
       if ((it-1)%200 == 0) 
        {   
            irec++ ;
            MPI_Barrier(MPI_COMM_WORLD); // Removing MPI Barrier after the calculations are done.
            write_to_file(u_b,v_b,h_b,myid,nx,my,ny,irec,dims,nproc) ; // writing the results.
        }  

     }  /* end  rk3 integration */
     
     free(dims);
     free(disps); //freeing the disps and dims arrays.
     toc = clock(); // time calculation 
     if(myid == 0) {
     printf("Elapsed: %f seconds\n", (double)(toc - tic) / CLOCKS_PER_SEC);}     

     free(h_a[0]); free(h_a); /*freeing the allocated arrays.*/                                                  
     free(h_b[0]); free(h_b);                                                   
    ...
    MPI_Finalize(); // Cleaning all states related to MPI.
   return 0; //returning 0 for the function.
}
